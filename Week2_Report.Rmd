...
We have a very nice result that after subtracting stop words, our English words % (over all tokens) are as high as 92% - 96%. Interestingly, blogs has a very high number of sentences per paragraphs (recognized by escape character "\\n") compared to the other source.  Tokens per sentence is about the same except twitter has a little bit shorter sentence (0.94 vs. 0.88).
...
Interestingly, __twitter__ and __blogs__ are very similar with words such as "like", "time", "love", "thanks", "one" etc.  However, as we can see, in __news__, a very significantly dominating word is "said".  

# Histogram of 2-Gram Terms

## Blogs

Let's also explore the 2-gram level terms. "Many people", "year old", "years ago" are the most frequent terms in __blogs__.  "Right now" and "just want" are the most frequent terms in __twitter__. "Health care", "years old" and "last year" are the most frequent terms in __news__,


```{r blog_tg_hist, echo = FALSE, cahce = TRUE}
...
```{r twitter_tg_hist, echo = FALSE, cahce = TRUE}
...
```{r news_tg_hist, echo = FALSE, cahce = TRUE}
